<x-admin.wrapper>
    <x-slot name="title">
        {{ __('O\'simlikni tahrirlash') }}
    </x-slot>

    <div>
        <x-admin.breadcrumb href="{{ route('admin.plants.index') }}" title="{{ __('Edit plant') }}">
            {{ __('<< Barcha o\'simliklar') }}
        </x-admin.breadcrumb>
        <x-admin.form.errors />
    </div>
    ...
    <div class="w-full py-2 overflow-hidden">
        <form method="POST" action="{{ route('admin.plants.update', $plant->id) }}" enctype="multipart/form-data">
            ...
            <div class="py-2">
                ...
                @if ($plant->image)
                    <div class="pb-2">
                        <img src="{{ asset('storage/' . $plant->image) }}" alt="plant Image" class="w-32 h-auto rounded shadow">
                    </div>
                @endif
